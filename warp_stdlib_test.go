...
func TestWarpStdlibEffectSamplesAndEmitsWGSL(t *testing.T) {
	root := "."
	result, err := compiler.CompileFile(filepath.Join(root, "effects", "warp_stdlib.lfx"), compiler.Options{
		BaseDir:  root,
	...
	if values[0] == values[1] && values[1] == values[2] {
		t.Fatalf("warp effect should vary across points, got %v", values)
	}
	...
	for _, marker := range []string{"warp__warp2"} {
